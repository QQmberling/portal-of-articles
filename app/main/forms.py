...
class UserEditForm(FlaskForm):
    first_name = StringField("Имя: ",
                             validators=[Length(max=30, message='Превышено ограничение в 30 символов для поля: Имя.')])
    last_name = StringField("Фамилия: ", validators=[
        Length(max=30, message='Превышено ограничение в 30 символов для поля: Фамилия.')])
    about = TextAreaField('Рассказать что-нибудь о себе', validators=[
        Length(max=230, message='Превышено ограничение в 230 символов для поля: О себе.')])
    avatar = FileField()
...
class ArticleCreateForm(FlaskForm):
    title = StringField('Название статьи',
                        validators=[DataRequired(), Length(max=60, message='Название не должно превышать 60 символов')])
    intro = TextAreaField('Описание статьи', validators=[DataRequired(), Length(max=230,
                                                                                message='Интро не должно превышать 230 символов')])
    text = TextAreaField('Содержимое статьи', validators=[DataRequired(), Length(max=30000,
                                                                                 message='Интро не должно превышать 30000 символов')])
    able_to_comment = BooleanField('Разрешить оставлять комментарии?', default=True)
...
class ArticleEditForm(FlaskForm):
    ...
    intro = TextAreaField('Описание статьи', validators=[DataRequired(), Length(max=230,
                                                                                message='Интро не должно превышать 230 символов')])
    text = TextAreaField('Содержимое статьи', validators=[DataRequired(), Length(max=30000,
                                                                                 message='Статья не должна превышать 30000 символов')])
